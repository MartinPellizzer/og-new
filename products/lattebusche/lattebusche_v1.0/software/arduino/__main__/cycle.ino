...
void cycle_start()
{
  if (cycle.state_on_old != cycle.state_on_cur)
  {
    ...
    cycle.working_millis_timer = (cycle.resting_millis_timer * power.power_cur) / 100;
    cycle.state_working_cur = 1;
    digitalWrite(RO_1, 1); 
    Serial.println("on");
    Serial.println(cycle.working_millis_timer);
  }
}
...
void cycle_run()
{
  ...
  {
    if (millis() - cycle.millis_cur > cycle.working_millis_timer) 
    {
      cycle.millis_cur = millis();
  ...
  {
    if (millis() - cycle.millis_cur > cycle.resting_millis_timer) 
    {
      cycle.millis_cur = millis();
      cycle.state_working_cur = 1;
      digitalWrite(RO_1, 1);
